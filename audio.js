...
window.SoundManager = {
    ...
    updateVolumes() {
        ...
        const targetMaster = window.playerStats.mute ? 0 : (window.playerStats.volumeMaster || 0.5);
        const targetSFX = window.playerStats.volumeSFX || 0.5;
        this.masterGain.gain.setTargetAtTime(Math.max(0, Math.min(1, targetMaster)), now, 0.015);
        this.sfxGain.gain.setTargetAtTime(Math.max(0, Math.min(1, targetSFX)), now, 0.015);
    },
    ...
    play(type) {
        ...
        switch (type) {
            case 'swing': this.synthesizeSwing(now, dest); break;
            case 'block': this.synthesizeBlock(now, dest); break;
            case 'parry': this.synthesizeParry(now, dest); break;
            case 'spell': this.synthesizeSpell(now, dest); break;
            case 'fairy': this.synthesizeFairy(now, dest); break;
            case 'death': this.synthesizeDeath(now, dest); break;
            case 'defeat': this.synthesizeDefeat(now, dest); break;
            case 'revive': this.synthesizeRevive(now, dest); break;
        }
    },
    ...
    synthesizeSwing(now, dest) {
        ...
        noiseFilter.frequency.exponentialRampToValueAtTime(window.randFloat(300, 450), now + duration);
        ...
        osc.frequency.exponentialRampToValueAtTime(window.randFloat(70, 95), now + duration);
        ...
        setTimeout(() => this.activeChannelCount = Math.max(0, this.activeChannelCount - 1), duration * 1000 + 40);
    },
    ...
    synthesizeParry(now, dest) {
        ...
        frequencies.forEach(f => {
            ...
            osc.frequency.linearRampToValueAtTime(f + window.randFloat(-10, 10), now + 0.18);
    ...
    synthesizeFairy(now, dest) {
        ...
        setTimeout(() => this.activeChannelCount = Math.max(0, this.activeChannelCount - 1), 250);
    },
    ...
    synthesizeRevive(now, dest) {
        ...
        const gainNode = this.ctx.createGain();
        gainNode.gain.setValueAtTime(0, now);
        gainNode.gain.linearRampToValueAtTime(0.20, now + 0.15);
        gainNode.gain.exponentialRampToValueAtTime(0.001, now + duration);
