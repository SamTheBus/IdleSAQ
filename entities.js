...
window.spawnPurchaseCelebration = function (theme, color, rarity) {
  ...
  if (rarity === 5 || rarity === "UNIQUE") {
    ...
    speed *= 1.4;
    text = rarity === "UNIQUE" ? "✨ UNIQUE TROPHY! ✨" : "🔥 MYTHIC PULL! 🔥";
  } else if (rarity === 4) {
...
window.drawSingleMob = function (c, m) {
  ...
  if (m.type === "mob") {
    ...
    if (vType === "slime") {
      ...
      c.beginPath();
      c.moveTo(cx - 5, cy - 2);
      c.quadraticCurveTo(cx - 18, cy - 12 - batWing, cx - 22, cy - 5 - batWing);
    ...
    for (let i = 0; i < 3; i++) {
      ...
      c.save();
      c.rotate(rot);
      c.strokeStyle = m.flashTimer > 0 ? "#ffffff" : "rgba(52, 152, 219, 0.45)";
      c.lineWidth = 1.5;
      c.beginPath();
        ...
        coinRows.forEach((row) => {
          ...
          for (let i = 1; i <= row.count; i++) {
            ...
            if (m.flashTimer === 0) {
              ...
              c.lineWidth = 0.8;
              c.beginPath();
              c.ellipse(0, 0, scaleW * 0.78, scaleH * 0.78, 0, 0, Math.PI * 2);
              c.stroke();
...
window.draw = function () {
  ...
  if (window.playerStats.isUberBoss) {
    ...
    if (tier === 0) {
      ...
      let cloudScroll2 = ((Date.now() / 400 + 300) % (canvas.width + 100)) - 50;
      ctx.beginPath();
      ...
      ctx.beginPath();
      ctx.ellipse(320, 170 + hover * 1.5, 45, 12, Math.PI / 12, 0, Math.PI * 2);
      ctx.ellipse(650, 140 - hover * 1.2, 35, 10, -Math.PI / 8, 0, Math.PI * 2);
      ctx.fill();
      ctx.stroke();
    } else if (tier === 5) {
      ...
      if (s.type === "tree") {
        ...
        if (tier === 0) {
          if (s.seed < 0.35) {
            ...
            ctx.beginPath();
            ctx.ellipse(tx, ty - 20 * ts, 16 * ts, 22 * ts, 0, 0, Math.PI * 2);
            ctx.fill();
            ctx.stroke();
          ...
          if (s.seed < 0.5) {
            ...
            ctx.quadraticCurveTo(tx - 10 * ts, ty, tx - 18 * ts, ty - 10 * ts);
  ...
  if (window.playerStats.isUberBoss) {
    ...
    if (window.playerStats.currentDungeon === "gold") {
      ...
      for (let i = -60; i < canvas.width + 60; i += 40) {
        ctx.fillRect(i - ((window.groundScroll + totalShift) % 40), 265, 8, 20);
      }
      ctx.beginPath();
      ctx.moveTo(0, 268);
    ...
    if (tier === 0) {
      ...
      for (let i = -200; i < canvas.width + 200; i += 220) {
        ...
        if (pop < 35) {
          ...
          ctx.strokeStyle = "rgba(46, 204, 113, " + (1 - (pop - 35) / 7) + ")";
          ctx.lineWidth = 1.5;
          ctx.beginPath();
          ctx.ellipse(
            gx + 60,
  ...
  if (window.reviveTimer > 0) {
    ...
    innerBeam.addColorStop(0.5, "rgba(255, 243, 176, " + 0.85 * (1 - t) + ")");
  ...
  if (
    ...
    let trailingPct = Math.max(
      0,
      Math.min(1, (window.mob.trailingHp || window.mob.hp) / window.mob.maxHp),
    );
    ctx.fillStyle = "#ffffff";
    ctx.beginPath();
    if (ctx.roundRect)
    ...
    if (window.mob.funnyTextTimer > 0 && window.mob.funnyText) {
      ...
      ctx.strokeStyle = "#000000";
      ctx.lineWidth = 3;
      ctx.strokeText(window.mob.funnyText, barX + barW / 2, barY + barH / 2);
      ctx.fillStyle = "#ffffff";
      ctx.fillText(window.mob.funnyText, barX + barW / 2, barY + barH / 2);
      ctx.restore();
    }
